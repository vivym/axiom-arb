...
#[tokio::test]
async fn strategy_history_repo_prefers_higher_history_seq_legacy_rows_when_timestamps_match() {
    let db = TestDatabase::new().await;
    run_migrations(&db.pool).await.unwrap();

    let adopted_at = chrono::DateTime::parse_from_rfc3339("2026-04-06T03:00:00Z")
        .unwrap()
        .with_timezone(&Utc);
    ...
    let latest = OperatorStrategyAdoptionHistoryRepo
        ...
        .await
        .unwrap()
        .expect("mixed-state repo should preserve history_seq ordering");

    assert_eq!(latest.adoption_id, "legacy-adoption-21");
    assert_eq!(latest.operator_strategy_revision, "targets-rev-21");
    assert_eq!(
        latest.previous_operator_strategy_revision.as_deref(),
        Some("targets-rev-20")
    );
...
#[tokio::test]
async fn strategy_history_repo_uses_higher_history_seq_legacy_previous_distinct_revision_when_timestamps_match(
) {
    let db = TestDatabase::new().await;
    run_migrations(&db.pool).await.unwrap();

    let adopted_at = chrono::DateTime::parse_from_rfc3339("2026-04-06T03:30:00Z")
        .unwrap()
        .with_timezone(&Utc);
    ...
    let previous = OperatorStrategyAdoptionHistoryRepo
        .previous_distinct_revision(&db.pool, "targets-rev-30")
        .await
        .unwrap();

    assert_eq!(previous.as_deref(), Some("targets-rev-29"));

    db.cleanup().await;
}

#[tokio::test]
async fn strategy_history_repo_prefers_higher_history_seq_even_when_legacy_row_has_older_adopted_at(
) {
    let db = TestDatabase::new().await;
    run_migrations(&db.pool).await.unwrap();
    ...
    .bind("candidate-40")
    .bind(
        chrono::DateTime::parse_from_rfc3339("2026-04-06T05:00:00Z")
            .unwrap()
            .with_timezone(&Utc),
    )
    .bind(30_i64)
    ...
    .bind("candidate-41")
    .bind(
        chrono::DateTime::parse_from_rfc3339("2026-04-06T04:00:00Z")
            .unwrap()
            .with_timezone(&Utc),
    )
    .bind(31_i64)
    ...
    let latest = OperatorStrategyAdoptionHistoryRepo
        ...
        .await
        .unwrap()
        .expect("mixed-state repo should preserve history_seq authority");

    assert_eq!(latest.adoption_id, "legacy-adoption-41");
    assert_eq!(latest.operator_strategy_revision, "targets-rev-41");
    assert_eq!(
        latest.previous_operator_strategy_revision.as_deref(),
        Some("targets-rev-40")
    );
...
#[tokio::test]
async fn strategy_history_repo_uses_higher_history_seq_previous_distinct_revision_even_when_legacy_row_has_older_adopted_at(
) {
    let db = TestDatabase::new().await;
    run_migrations(&db.pool).await.unwrap();
    ...
    .bind("candidate-50")
    .bind(
        chrono::DateTime::parse_from_rfc3339("2026-04-06T06:00:00Z")
            .unwrap()
            .with_timezone(&Utc),
    )
    .bind(40_i64)
    ...
    let previous = OperatorStrategyAdoptionHistoryRepo
        .previous_distinct_revision(&db.pool, "targets-rev-50")
        .await
        .unwrap();

    assert_eq!(previous.as_deref(), Some("targets-rev-49"));
