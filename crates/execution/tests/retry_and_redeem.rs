...
fn order_envelope_carries_the_attempt_context_used_to_create_it() {
    ...
    let (attempt, context) = factory.next_for_plan(&plan, &request, request.activation_mode);
...
fn execution_attempt_factory_resets_attempt_numbers_per_request_bound_plan_identity() {
    ...
    let (attempt_a1, _) = factory.next_for_plan(&plan_a, &request_a, request_a.activation_mode);
    let (attempt_a2, _) = factory.next_for_plan(&plan_a, &request_a, request_a.activation_mode);
    let (attempt_request_b1, _) =
        factory.next_for_plan(&plan_a, &request_b, request_b.activation_mode);
    let (attempt_plan_b1, _) =
        factory.next_for_plan(&plan_b, &request_b, request_b.activation_mode);
...
fn execution_attempt_factory_keeps_same_business_plan_independent_across_requests() {
    ...
    let (attempt_a, _) = factory.next_for_plan(&plan, &request_a, request_a.activation_mode);
    let (attempt_b, _) = factory.next_for_plan(&plan, &request_b, request_b.activation_mode);
