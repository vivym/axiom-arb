...
async fn replay_orders_by_journal_seq_and_preserves_replay_fields() {
    let writer = JournalWriter::in_memory();
    writer.append(sample_market_event()).await.unwrap();
    writer.append(sample_user_event()).await.unwrap();
    ...
    assert_eq!(replay[0].source_event_id.as_deref(), Some("book-1"));
    ...
    assert_eq!(replay[1].causal_parent_id.as_deref(), Some("book-1"));
}
...
fn sample_market_event() -> JournalEvent {
    JournalEvent {
        ...
        source_event_id: Some("book-1".to_owned()),
