...
`apply` is the Day 1+ smoke path. It reuses `status`, can inline an explicit adopt when the target anchor is missing, can explicitly enable the smoke-only rollout posture for the adopted families, runs `doctor`, and stops at ready unless you also pass `--start`. It does not chain `verify`; run that explicitly after the runtime work you want to inspect.
