...
6. Adopt a startup-scoped operator target revision with `cargo run -p app-live -- targets adopt --config config/axiom-arb.local.toml --adoptable-revision <ADOPTABLE_REVISION>`. When the rollout lists are empty, this only writes `operator_target_revision`; rollout readiness remains separate work unless bootstrap explicitly enables the smoke-only rollout posture for the adopted family set.
7. Run `cargo run -p app-live -- doctor --config config/axiom-arb.local.toml`.
8. Start the daemon with `cargo run -p app-live -- run --config config/axiom-arb.local.toml`, or let `cargo run -p app-live -- bootstrap --start` enter `run` after the bootstrap flow reaches a ready state.
...
`bootstrap` is now the preferred high-level path for `paper` and `real-user shadow smoke`. For smoke it keeps startup authority on `[negrisk.target_source].operator_target_revision`, inlines explicit adoption only when no target anchor exists yet, and then distinguishes two outcomes: `preflight-ready smoke startup` when rollout readiness is still empty, and `shadow-work-ready smoke startup` when bootstrap explicitly enables smoke-only rollout for the adopted family set. `init` remains available as the lower-level config wizard, and `targets ...` remain the lower-level control-plane commands. `doctor` is the real preflight gate for `live` and `real-user shadow smoke`: it reports `Config`, `Credentials`, `Connectivity`, `Target Source`, and `Runtime Safety` sections, performs venue-facing probes for authenticated REST, market ws, user ws, heartbeat, and relayer reachability when those checks apply, and ends with explicit next actions. `targets adopt` and `targets rollback` rewrite `[negrisk.target_source].operator_target_revision` in the local TOML and record adoption history, but they remain startup-scoped operations. They do not hot-reload a running daemon. Use `targets status`, `targets show-current`, and `doctor` to check whether `configured_operator_target_revision` differs from the daemon's active revision and whether a controlled restart is required.
