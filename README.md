...
3. Prefer `cargo run -p app-live -- bootstrap` for Day 0 `paper` and `real-user shadow smoke` setup. It defaults to `config/axiom-arb.local.toml`, reuses the init wizard, and for smoke it now runs discovery first when the database has no artifacts. From there it either lists adoptable revisions and waits for explicit selection, or it truthfully stops with a no-adoptable discovery summary.
...
5. If you want the lower-level Day 0 fallback instead of `bootstrap`, run `cargo run -p app-live -- init --config config/axiom-arb.local.toml` to create the operator-local config and complete the interactive prompts for the long-lived account and relayer auth values, then run `cargo run -p app-live -- discover --config config/axiom-arb.local.toml`, inspect persisted artifacts with `cargo run -p app-live -- targets candidates --config config/axiom-arb.local.toml`, and adopt with `cargo run -p app-live -- targets adopt --config config/axiom-arb.local.toml --adoptable-revision <ADOPTABLE_REVISION>`.
6. Re-run `cargo run -p app-live -- status --config config/axiom-arb.local.toml` and follow its next action. When rollout lists are empty, adoption alone only writes `operator_target_revision`; rollout readiness remains separate work unless `bootstrap` or `apply` explicitly enables the smoke-only rollout posture for the adopted family set.
7. Discovery artifacts and adoption lineage are separate facts. `discover` persists advisory candidate and adoptable artifacts only. `targets adopt` writes the canonical adoption provenance for the chosen `operator_target_revision` and appends adoption history.
8. Run `cargo run -p app-live -- doctor --config config/axiom-arb.local.toml` once `status` says the config is ready for preflight, or let `apply` run `doctor` for you in the high-level smoke path.
9. Start the daemon with `cargo run -p app-live -- run --config config/axiom-arb.local.toml`, or let `cargo run -p app-live -- bootstrap --start` / `cargo run -p app-live -- apply --config config/axiom-arb.local.toml --start` enter `run` when those high-level flows reach a ready state.
...
`bootstrap` remains the preferred Day 0 path for `paper` and `real-user shadow smoke`. The low-level Day 0 fallback is `discover -> targets candidates -> targets adopt`. `apply` is the Day 1+ smoke orchestration path: it reuses `status`, can inline explicit smoke target adoption only after discovery has already produced adoptable artifacts, can explicitly enable smoke-only rollout for the adopted family set, runs `doctor`, and with `--start` can enter foreground `run` after an explicit manual restart-boundary confirmation when needed. `status` is the operator homepage for config-and-control-plane readiness: it tells you whether you still need discovery, adoption, rollout enablement, a controlled restart, `doctor`, or `run`, without doing any venue probes. For smoke, `bootstrap` keeps startup authority on `[negrisk.target_source].operator_target_revision`, and `apply` continues from there without introducing any new authority. `run` creates a durable `run_session`; `status` now surfaces both the latest relevant `run_session` for the current config and any conflicting active run, and it projects `stale` from freshness instead of storing it as a separate truth value. `verify` is the post-run result check: it uses only local evidence, does not do venue probes, and answers whether the latest local result matches `paper-no-live`, `smoke-shadow-only`, or conservative `live-config-consistent` expectations. When `verify` cannot uniquely map a historical window to a session, it downgrades to evidence-only rather than pretending to know the historical control-plane truth. In the operator output, `Run session`, `Relevant run session`, and `Conflicting active run session` are the stable handles to look for. `targets adopt` and `targets rollback` rewrite `[negrisk.target_source].operator_target_revision` in the local TOML, write or reuse canonical adoption provenance, and append adoption history, but they remain startup-scoped operations. They do not hot-reload a running daemon. Use `status` first, then drop into `apply`, `targets ...`, `doctor`, or `verify` only when the high-level next action tells you to.
