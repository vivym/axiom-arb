...
4. For Day 1+ `real-user shadow smoke` progression, follow `cargo run -p app-live -- status --config config/axiom-arb.local.toml -> cargo run -p app-live -- apply --config config/axiom-arb.local.toml [--start] -> cargo run -p app-live -- verify --config config/axiom-arb.local.toml`. `apply` is the conservative live path: it stays downstream of adoption and rollout posture, stops at ready, and only continues into foreground `run` when you add `--start` and confirm the manual restart boundary.
...
6. If `status` reports `target-adoption-required` and you want lower-level control-plane control, inspect candidate/adoptable state with `cargo run -p app-live -- targets candidates --config config/axiom-arb.local.toml`, then adopt a startup-scoped operator target revision with `cargo run -p app-live -- targets adopt --config config/axiom-arb.local.toml --adoptable-revision <ADOPTABLE_REVISION>`. `apply` does not inline that adoption step.
7. Re-run `cargo run -p app-live -- status --config config/axiom-arb.local.toml` and follow its next action. When rollout lists are empty, adoption alone only writes `operator_target_revision`; rollout readiness remains separate work until the rollout posture already exists.
8. Run `cargo run -p app-live -- doctor --config config/axiom-arb.local.toml` once `status` says the config is ready for preflight, or let `apply` run `doctor` for you after readiness is already in place.
