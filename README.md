...
`bootstrap` is now the preferred high-level path for `paper` and `real-user shadow smoke`. The intended high-level flow is `bootstrap -> status -> doctor -> run -> verify`. `status` is the operator homepage for config-and-control-plane readiness: it tells you whether you still need target adoption, rollout enablement, a controlled restart, `doctor`, or `run`, without doing any venue probes. For smoke, bootstrap keeps startup authority on `[negrisk.target_source].operator_target_revision`, inlines explicit adoption only when no target anchor exists yet, and then distinguishes two outcomes: `preflight-ready smoke startup` when rollout readiness is still empty, and `shadow-work-ready smoke startup` when bootstrap explicitly enables smoke-only rollout for the adopted family set. `init` remains available as the lower-level config wizard, `targets ...` remain the lower-level control-plane commands, and `targets status` / `targets show-current` remain the detailed provenance views. `doctor` is still the real preflight gate for `live` and `real-user shadow smoke`: it reports `Config`, `Credentials`, `Connectivity`, `Target Source`, and `Runtime Safety` sections, performs venue-facing probes for authenticated REST, market ws, user ws, heartbeat, and relayer reachability when those checks apply, and ends with explicit next actions. `verify` is the post-run result check: it uses only local evidence, does not do venue probes, and answers whether the latest local result matches `paper-no-live`, `smoke-shadow-only`, or conservative `live-config-consistent` expectations. `targets adopt` and `targets rollback` rewrite `[negrisk.target_source].operator_target_revision` in the local TOML and record adoption history, but they remain startup-scoped operations. They do not hot-reload a running daemon. Use `status` first, then drop into `targets ...`, `doctor`, or `verify` only when the high-level next action tells you to.
