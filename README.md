...
4. For Day 1+ `real-user shadow smoke` progression, check readiness with `cargo run -p app-live -- status --config config/axiom-arb.local.toml`, then use `cargo run -p app-live -- apply --config config/axiom-arb.local.toml` to inline smoke-only adoption and rollout work and stop at ready. Add `--start` only when you want the same flow to continue into foreground `run`.
...
`bootstrap` remains the preferred Day 0 path for `paper` and `real-user shadow smoke`. `apply` is the Day 1+ smoke orchestration path: it reuses `status`, can inline explicit smoke target adoption, can explicitly enable smoke-only rollout for the adopted family set, runs `doctor`, and with `--start` can enter foreground `run` after an explicit manual restart-boundary confirmation when needed. `status` is the operator homepage for config-and-control-plane readiness: it tells you whether you still need target adoption, rollout enablement, a controlled restart, `doctor`, or `run`, without doing any venue probes. For smoke, `bootstrap` keeps startup authority on `[negrisk.target_source].operator_target_revision`, and `apply` continues from there without introducing any new authority. `init` remains available as the lower-level config wizard, `targets ...` remain the lower-level control-plane commands, and `targets status` / `targets show-current` remain the detailed provenance views. `doctor` is still the real preflight gate for `live` and `real-user shadow smoke`: it reports `Config`, `Credentials`, `Connectivity`, `Target Source`, and `Runtime Safety` sections, performs venue-facing probes for authenticated REST, market ws, user ws, heartbeat, and relayer reachability when those checks apply, and ends with explicit next actions. `verify` is the post-run result check: it uses only local evidence, does not do venue probes, and answers whether the latest local result matches `paper-no-live`, `smoke-shadow-only`, or conservative `live-config-consistent` expectations. `targets adopt` and `targets rollback` rewrite `[negrisk.target_source].operator_target_revision` in the local TOML and record adoption history, but they remain startup-scoped operations. They do not hot-reload a running daemon. Use `status` first, then drop into `apply`, `targets ...`, `doctor`, or `verify` only when the high-level next action tells you to.
